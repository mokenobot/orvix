document.addEventListener('DOMContentLoaded', () => {
    // API CONFIGURATION (Points to your Hidencloud Host)
    const API_BASE = 'http://zara.hidencloud.com:24631';
    ...
    async function initDashboard() {
        try {
            // Fetch User Guilds
            const response = await fetch(`${API_BASE}/api/user/guilds`, {
    ...
    async function openSettings(guild) {
        ...
        try {
            const response = await fetch(`${API_BASE}/api/config/${guild.id}`, {
                headers: { 'Authorization': `Bearer ${currentToken}` }
            });
    ...
    saveConfigBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE}/api/config/${activeGuildId}`, {
                method: 'POST',
                headers: {
